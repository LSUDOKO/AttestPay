...
export function cardUrl(secret: string): string {
  const base = process.env.ATTESTPAY_PUBLIC_MCP_BASE ?? `http://localhost:${process.env.PORT ?? 4070}`;
 ...
 * no private/loopback/link-local targets unless ATTESTPAY_PAID_FETCH_ALLOW_LOCAL=1 (dev). */
function ssrfGuard(rawUrl: string): void {
  ...
  const allowLocal = process.env.ATTESTPAY_PAID_FETCH_ALLOW_LOCAL === "1";
