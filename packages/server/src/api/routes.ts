...
//   - ADMIN (ops): Bearer ATTESTPAY_ADMIN_TOKEN · full access, server-side curl/scripts.
...
//     "attestpay-onboard:v1:<did>" (personal_sign), so a Privy login can only ever claim
...
export function apiRoutes(deps: AppDeps, oauth: OAuthStore): Hono<ApiEnv> {
  ...
  app.use("*", async (c: Context<{ Variables: { auth: AuthCtx } }>, next: Next) => {
    if (!deps.adminToken && !deps.verifyPrivyToken) {
      return c.json({ error: "api disabled (no ATTESTPAY_ADMIN_TOKEN and no ATTESTPAY_PRIVY_APP_ID)" }, 503);
    }
  ...
  // over "attestpay-onboard:v1:<did>"): possession of the key, bound to THIS login.
  app.post("/onboard", (c) =>
    handle(c, async () => {
  ...
  app.post("/cards", (c) =>
    handle(c, async () => {
      ...
      if (!deps.userSigner) throw new EngineError("api", "no dev signer configured (ATTESTPAY_DEV_USER_PK)");
